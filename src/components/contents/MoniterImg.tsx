...
export default function MoniterImg({ idx }: IMoniterImg) {
  return (
    <MoniterWrapper>
      <PCMoniter style={{ backgroundColor: backColors[idx] }}>
        <PcImg
          src={`${process.env.PUBLIC_URL}/works/${workList[idx].img[0]}.png`}
        />
      </PCMoniter>
      {workList[idx]?.img[1] && (
        <MobileMoniter style={{ backgroundColor: backColors[idx] }}>
          <MobileImg
            src={`${process.env.PUBLIC_URL}/works/${workList[idx].img[1]}.png`}
          />
        </MobileMoniter>
      )}
    </MoniterWrapper>
