...
export const workInfo = [
  ...
  },
  {
    ...
    issue: [
      '현재 시간을 받아온 후 setInterval을 통해 1초씩 시간을 변경하는 함수를 만들어 시간을 표시하였으나, 네트워크 환경에 따라 시간이 실제와 달라지는 버그가 발생 <br/> -> 현재 시간을 가져오는 함수 getClock을 만든 후 앱 실행과 동시에 getClock을 볼러와 시간을 표시한 후 setInterval로 해당 함수를 1초에 한번씩 불러오는 방법으로 변경',
      '날씨 정보를 받아오려면 현재 나의 정확한 위치를 알아야함<br/> -> Event handler에서 기본으로 제공하는 event를 통해 coords.latitude, coords.longitude로 현재 위치를 받아옴 -> 내가 있는 동네 이름과 날씨 두가지 다 제공할 수 있게 되었음',
    ],
...
export const playvoiceWork = {
  ...
  list: [
    ...
    },
    {
      ...
      des: [
        ...
        '1. Eng-stack: 해석이 궁금하거나 공유하고 싶은 짧은 영어 문장을 등록하고 회원들과 함께 해성하는 공간. 예문을 등록시 기본적으로 3가지의 AI들이 해석을 해줌.',
  ...
  issue: [
    '네이버만큼이나 많은 데이터와 화면 UI들로 인해 JavaScript로 작업해야하는 부분이 많았음. 해당 코드들을 선언형 스타일로 작성하였기 때문에 가독성이 떨어지며 유지보수하기가 불편함.<br/> -> 주석 규칙을 정하고 서버와 통신하는 JS 코드는 js 파일로 따로 분리했으며 dom을 handling 하는 JS 코드들은 HTML 파일 맨 위에 순서대로 배치',
    '대부분의 등록, 수정, 삭제 과정을 비동기로 진행하였음. 회원이 컨텐츠를 등록하면 즉시 컨텐츠를 화면에 보여주며, 영어예문은 영어 음성을 만들어 바로 voice로 들려줘야하는 어려움이 있었음.<br/> -> playvoice_audio.js라는 공통 오디오 모듈을 만들어 버튼에 정해진 class 명만 붙이면 예문이 재생되도록 구현하여 해결. 비동기로 화면에 등록되는 HTML 코드의 비어있는 버전을 파일 가장 하단에 배치하여 clone을 한 후 컨텐츠를 넣는 방식으로 해결',
    '유료 멤버십을 위한 결제 모듈 넣기, 결제 모듈 창을 띄우기 위해 필요한 정보, 리다이렉트로 필요한 페이지들 등<br/> -> 결제 모듈 기능을 팀장인 저는 전체를, 팀원 각자는 맡은 부분을 공부한 후 각자 발표를 함. 이니시스 계약 후 발표한 자료에 따라 화면, 서버, js 파일 순으로 개발 진행. 가격은 100원으로 책정한 후 약 50번 이상의 테스트 후 완벽히 구동시킴',
    '회원들이 영어문장을 등록한 후 단어에 빨강색으로 강조 표시를 하는 입력란이 많음. 입력란을 input이 아닌 div로 만들고, 대부분 회원들이 영어 문장을 복사해오기때문에 HTML 태그가 그대로 입력되거나 영어가 아닌 문자열이 입력되는 경우가 많았음.<br/> -> 클립보드의 스타일 서식을 모두 지우고 순수 텍스트만 붙여넣기 되도록 구현. 해당 구현을 모듈로 따로 만들어 필요한 화면에 넣어 해결',
  ],
  use:
